...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const cardSize = Math.min(this.viewport.width, this.viewport.height) * this.itemSize;
...
class App {
  constructor(
    ...
    this.sourceItemCount = items?.length || 0;
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend, textColor, borderRadius, font, itemSize);
    this.update();
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font, itemSize = 0.36) {
    const defaultItems = [
  ...
  getActiveIndex() {
    if (!this.medias?.[0] || !this.sourceItemCount) return 0;
    const width = this.medias[0].width;
    const raw = Math.round(Math.abs(this.scroll.current) / width);
    return raw % this.sourceItemCount;
  }

  emitActiveChange() {
    if (!this.onActiveChange || !this.sourceItemCount) return;
    const active = this.getActiveIndex();
    if (active === this.lastActiveIndex) return;
    this.lastActiveIndex = active;
    this.onActiveChange(active);
  }

  scrollToIndex(index) {
    if (!this.medias?.[0] || !this.sourceItemCount) return;
    const safe = ((index % this.sourceItemCount) + this.sourceItemCount) % this.sourceItemCount;
    const width = this.medias[0].width;
    this.scroll.target = width * safe;
    this.scroll.current = this.scroll.target;
    this.lastActiveIndex = safe;
    this.onActiveChange?.(safe);
  }
  onResize() {
    this.screen = {
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, itemSize]);

  useEffect(() => {
    if (focusedIndex == null || !appRef.current) return;
    appRef.current.scrollToIndex(focusedIndex);
  }, [focusedIndex]);
